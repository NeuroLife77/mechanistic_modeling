...
class NodeModelSampler(BaseSampler):
    def __init__(
                    self, model,
                    data_sample_set:DataSample, 
                    alpha:AlphaNodeModel,
                    S = None,
    ...
    def _get_model_info(self):
        ...
        info = self.model(parameters=None,S=0)
        ...
        if not is_output_ok:
            ...
            for data_sample_set_feature_name in self.data_sample_set.T_feature_names:
                ...
                if not has_matching_subset:
                    still_not_ok = True
            if still_not_ok:
                raise AttributeError(f"{self.__class__}._get_model_info(): \t data_sample_set and Model features don't match \n \t data_sample_set Features = {self.data_sample_set.T_feature_names} \n \t Model Features = {self.model_output}")
            else:
                print(f"{self.__class__}._get_model_info(): \t Found that some model feature names are subsets of multiple data_sample_set feature names, assuming that it is intentional. Will crash if not.")
    ...
    def get_samples(
        ...
        if self.parallelize_sim:
            results = Parallel(n_jobs = self.max_jobs, verbose = verbose)(delayed(self.model)(
                                                                            **job_info,
                                                                            **self.model_kwargs,
                                                                            **job_info["kwargs"],
        ...
        if self.n_data_sample_set > 0:
            ...
            #print("run_gof_shape",(run_log["gof"]).shape)
    ...
    def _sample_initial_conditions(self,n_init_samples, **kwargs):
        init_cond_range = (self.state_var_init_range[1]-self.state_var_init_range[0])[None,:]
        init_cond_min = (self.state_var_init_range[0])[None,:]
    ...
    def _sample_initial_conditions_uniform(self, n_init_samples, **kwargs):
        return self.sample_generator.uniform(0,1,size=(n_init_samples,self.n_state_vars))
    
    def _sample_initial_conditions_beta(self, n_init_samples, beta_params = [2,3], **kwargs):
        return self.sample_generator.beta(beta_params[0],beta_params[1],size=(n_init_samples,self.n_state_vars))
    def _sample_initial_conditions_gaussian(self,n_init_samples, mu=0.5,sigma=0.15, **kwargs):
        return self.sample_generator.normal(mu,sigma,size=(n_init_samples,self.n_state_vars))

class SimulatedAnnealingSamplerNode:
    def __init__(self,
                model_sampler:NodeModelSampler,
                theta_center = None,
